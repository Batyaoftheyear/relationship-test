(() => {
  ...
  function getFlexiblePartnershipProfile(result) {
    ...
    if (result.normalized.x >= 10) {
      return `Ваш результат попадает в зону гибкого партнёрства главным образом за счёт партнёрского взгляда на горизонтальной оси. Вы скорее не сводите отношения к конкуренции и контролю: для вас важны доверие, распределение ответственности и возможность договариваться. При этом по вертикальной оси позиция ближе к центру, поэтому вера в изменяемость не выглядит безусловной: исходные данные, статус и среда тоже могут восприниматься как значимые ограничения. ${centerNuance}`;
    }
