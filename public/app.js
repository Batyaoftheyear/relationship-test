(() => {
  ...
  const QUADRANT_META = {
    ...
    'Фиксированная конкуренция': {
      description: 'Сочетание конкуренции и детерминизма, где исход отношений воспринимается как в основном предопределённый.',
  ...
  function getAxisXProfile(x) {
    if (x >= 35) {
      return {
        ...
        meaning: 'Вы скорее воспринимаете отношения как пространство договорённостей, взаимного учёта интересов и согласования границ.',
        nuance: 'Конфликты в такой картине не исчезают, но решаются не через постоянную борьбу за верхнюю позицию, а через обсуждение условий и ответственности.'
      };
    }
    ...
    if (x >= 10) {
      return {
        ...
        meaning: 'Вам ближе партнёрская логика, где важны доверие, разговор и распределение ответственности.',
        nuance: 'При этом результат не выглядит наивным: вы, вероятно, допускаете, что в отношениях бывают интересы, давление и неравномерное распределение влияния.'
      };
    }
    ...
    if (x >= -9) {
      return {
        ...
        meaning: 'Вы не занимаете жёстко партнёрскую или жёстко конкурентную позицию.',
        nuance: 'Скорее вы различаете ситуации: где-то уместны открытость и договорённости, а где-то нужны осторожность, рамки и понимание интересов сторон.'
      };
    }
    ...
    if (x >= -34) {
      return {
        ...
        meaning: 'Вы чаще смотрите на отношения как на среду, где важно не терять позицию, видеть интересы сторон и управлять динамикой взаимодействия.',
        nuance: 'Это не обязательно означает отказ от близости или договорённостей, но доверие для вас, вероятно, не является достаточным основанием само по себе.'
      };
    }

    return {
      label: 'выраженный сдвиг к конкуренции',
      meaning: 'Вы склонны видеть отношения через призму стратегии, контроля, обмена условий и борьбы за влияние.',
      nuance: 'В такой картине устойчивость чаще зависит от умения удерживать позицию и читать скрытые правила взаимодействия, чем от открытого согласования намерений.'
    };
  }
  ...
  function getQuadrantProfile(result, xProfile, yProfile) {
    ...
    const descriptions = {
      ...
      'Стратегическая конкуренция': `Ваш результат ближе к стратегической конкуренции. В этой позиции взаимодействие мужчины и женщины чаще выглядит как среда интересов, статуса и влияния, но при этом человек всё ещё может менять результат через навыки, стратегию и поведение. ${getCenterNuance(result)}`,
      ...
      'Фиксированная конкуренция': `Ваш результат ближе к фиксированной конкуренции. В этой позиции отношения чаще воспринимаются как конкурентная среда, где многое задаётся исходными условиями, статусом, привлекательностью и распределением влияния. ${getCenterNuance(result)}`
    };
  ...
  function getFlexiblePartnershipProfile(result) {
    ...
    if (result.normalized.x >= 35 && result.normalized.y >= 35) {
      return `Ваш результат ближе к выраженному гибкому партнёрству. В такой картине отношения не выглядят как рынок, игра на подавление или борьба за контроль: важнее способность двух людей проговаривать намерения, договариваться о правилах и менять поведение, если прежний формат перестал работать. Вы скорее считаете, что доверие, ясность ожиданий и взаимная настройка дают больше, чем постоянные проверки и удержание силового баланса. ${centerNuance}`;
    }

    if (result.normalized.x >= 10 && result.normalized.y >= 10) {
      return `Ваш результат ближе к гибкому партнёрству. Это не позиция про «всё решается разговорами» и не отрицание реальных ограничений, а скорее ставка на управляемость отношений: люди могут замечать свои интересы, обсуждать правила взаимодействия и корректировать поведение вместо того, чтобы сразу читать отношения как борьбу за ресурс. В знакомстве и сближении для вас, вероятно, важны ясность, уважение границ и способность учиться на обратной связи. ${centerNuance}`;
    }

    if (result.normalized.x >= 10) {
      return `Ваш результат попадает в зону гибкого партнёрства главным образом за счёт партнёрского взгляда на горизонтальной оси. Вы скорее не сводите отношения к конкуренции и контролю: для вас важны доверие, распределение ответственности и возможность договариваться. При этом по вертикальной оси позиция ближе к центру, поэтому акцент на агентности не выглядит безусловным: исходные данные, статус и среда тоже могут восприниматься как значимые ограничения. ${centerNuance}`;
    }

    if (result.normalized.y >= 10) {
      return `Ваш результат попадает в зону гибкого партнёрства главным образом за счёт сдвига к агентности. Вы скорее считаете, что поведение, навыки общения и готовность менять стратегию способны улучшать динамику знакомства и отношений. При этом по горизонтальной оси позиция близка к центру: вы не полностью исключаете конкурентные мотивы, интересы сторон и необходимость держать рамки, но не делаете их главным объяснением отношений. ${centerNuance}`;
    }

    return `Ваш результат формально относится к гибкому партнёрству, но находится близко к центру. Это скорее мягкий уклон, чем жёсткая позиция: вы допускаете ценность договорённостей, доверия и личного влияния на ситуацию, но, вероятно, сильно учитываете контекст конкретного человека, стадии знакомства и распределения интересов. ${centerNuance}`;
  }
